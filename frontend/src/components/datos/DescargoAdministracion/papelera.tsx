...
import { Trash2, RotateCcw, AlertCircle, Calendar, Search, X, ClipboardList, User, DollarSign, Package } from "lucide-react";
import { MostrarPapeleraDescargos, RestaurarDescargo, EliminarDefinitivoDescargo } from "../../conexion/DescargoAdministracion/Papelera";
import { MostrarPacientes, type Paciente } from "../../conexion/Paciente/Mostrar";
import { MostrarMedicamentosReal, type Medicamento } from "../../conexion/Inventario/Mostrar";
import PopoverAnimado from "../../common/PopoverAnimado";
import { useToast } from "../../common/ToastContext";

export function DescargoAdministracionPapelera({ theme }: any) {
  ...
  const loadData = async () => {
    try {
      const [d, p, m] = await Promise.all([MostrarPapeleraDescargos(), MostrarPacientes(), MostrarMedicamentosReal()]);
  ...
  const mMap = medicamentos.reduce((acc, m) => { acc[m.codm] = m; return acc; }, {} as Record<number, Medicamento>);

  const handleRestore = async (id: number) => {
    try { await RestaurarDescargo(id); showToast("Descargo restaurado.", "success"); loadData(); window.dispatchEvent(new Event("update-borrados-count"));}
    catch { showToast("Error al restaurar.", "warning"); }
  };
  const handleDelete = async (id: number) => {
    try { await EliminarDefinitivoDescargo(id); showToast("Eliminado permanentemente.", "success"); loadData(); window.dispatchEvent(new Event("update-borrados-count"));}
    catch { showToast("Error al eliminar.", "warning"); }
  };

  const filtered = deleted.filter(d => {
    const p = pMap[d.idp]; const m = mMap[d.codm];
    return `${d.idd} ${p ? `${p.nom} ${p.ap}` : ""} ${m ? m.nomm : ""}`.toLowerCase().includes(searchTerm.toLowerCase());
  });
    ...
    <div className="flex flex-col h-full p-2 pt-0 overflow-hidden">
      <div className={`flex flex-col md:flex-row justify-between sticky top-0 z-19 items-start md:items-center p-4 gap-4 mb-3 shrink-0 ${isDarkMode ? "bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800" : "bg-white/90 backdrop-blur-md border-b border-gray-100"}`}>
        <div className="flex items-center gap-3 text-red-500">
          <div className="p-3 bg-red-500/10 rounded-2xl"><ClipboardList size={26} /></div>
          <div>
            <h1 className="text-2xl font-black tracking-tight">Papelera de Descargos</h1>
            <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${isDarkMode ? "text-zinc-500" : "text-gray-400"}`}>
              {deleted.length} Descargo{deleted.length !== 1 ? "s" : ""} eliminado{deleted.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        <div className="flex-1 md:flex-none md:w-80 relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={18} />
          <input type="text" placeholder="Buscar paciente o fármaco..." className={`${inputBase} pl-12 pr-10`}
            autoComplete="off"
            value={searchTerm} onChange={e => setSearchTerm(e.target.value)} list="papelera-desc-list" />
          <datalist id="papelera-desc-list">
            {[...new Map(
              deleted.filter(d => pMap[d.idp]).map(d => [d.idp, pMap[d.idp]])
            ).values()].slice(0, 5).map(p => (
              <option key={p.ci} value={`${p.nom} ${p.ap}`} />
            ))}
            {[...new Map(
              deleted.filter(d => mMap[d.codm]).map(d => [d.codm, mMap[d.codm]])
            ).values()].slice(0, 5).map(m => (
              <option key={m.codm} value={m.nomm} />
            ))}
          </datalist>
          <AnimatePresence>{(
            <motion.button
            ...
            <tbody>
              <AnimatePresence mode="popLayout">
                {filtered.length > 0 ? filtered.map((d, i) => (
                  <motion.tr key={d.idd} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: i * 0.03 }}
                    className={`border-b last:border-0 ${isDarkMode ? "border-zinc-900 hover:bg-zinc-900/40" : "border-gray-50 hover:bg-gray-50/50"}`}>
                    <td className="px-6 py-5"><span className="text-[11px] font-mono font-bold bg-red-500/10 text-red-500 px-2 py-1 rounded-lg">#{d.idd}</span></td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-red-500/10 flex items-center justify-center text-red-400"><User size={16} /></div>
                        <div>
                          <p className="font-black text-sm tracking-tight capitalize">{pMap[d.idp] ? `${pMap[d.idp].nom} ${pMap[d.idp].ap}` : `CI: ${d.idp}`}</p>
                          <p className="text-[10px] opacity-40 uppercase font-bold">CI: {d.idp}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-black text-sky-500 uppercase tracking-tight">{mMap[d.codm] ? mMap[d.codm].nomm : `Cód: ${d.codm}`}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center"><span className={`text-sm font-black px-2 py-1 rounded-lg ${isDarkMode ? "bg-zinc-800 text-zinc-300" : "bg-gray-100 text-gray-600"}`}>{d.cant}</span></td>
                    <td className="px-6 py-5 text-center"><span className="flex items-center justify-center gap-0.5 text-xs font-black text-emerald-500"><DollarSign size={12} />{Number(d.costo).toFixed(2)}</span></td>
                    <td className="px-6 py-5"><p className={`text-xs font-bold italic ${isDarkMode ? "text-zinc-500" : "text-gray-400"}`}>{d.UsuarioElimino || "Sistema"}</p></td>
                    <td className="px-6 py-5">
                      <div className="flex flex-col items-center gap-0.5 text-[10px] font-bold text-zinc-500">
                        <div className="flex items-center gap-1.5"><Calendar size={11} className="text-red-500/60" />
                          {d.FechaEliminacion ? new Date(d.FechaEliminacion).toLocaleDateString("es-ES", { day: "2-digit", month: "2-digit", year: "numeric" }) : "—"}
                        </div>
                        <span className="opacity-50">{d.FechaEliminacion ? new Date(d.FechaEliminacion).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" }) : ""}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex justify-center gap-2">
                        <PopoverAnimado id={`restore-desc-${d.idd}`} titulo="Restaurar Descargo" direccion="bl" pWidth={355} pHeight={245} theme={theme} triggerWidth={42} triggerHeight={42}
                          ...
                          {({ close }: any) => (
                            <div className="flex flex-col h-full">
                              <p className={`text-sm mb-6 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}>¿Restaurar el descargo <strong>#{d.idd}</strong> al registro activo?</p>
                              <div className="flex gap-3 mt-auto">
                                ...
                                <button onClick={() => { handleRestore(d.idd); close(); }} className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg shadow-emerald-500/20 active:scale-95">Restaurar</button>
                              </div>
                            </div>
                          )}
                        </PopoverAnimado>
                        <PopoverAnimado id={`delete-desc-${d.idd}`} titulo="Eliminar Permanentemente" direccion="bl" pWidth={480} pHeight={245} theme={theme} triggerWidth={42} triggerHeight={42}
                          ...
                          {({ close }: any) => (
                            <div className="flex flex-col h-full">
                              <p className={`text-sm mb-6 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}>¿Eliminar permanentemente el descargo <strong>#{d.idd}</strong>?</p>
                              <div className="flex gap-3 mt-auto">
                                <button onClick={() => close()} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase ${isDarkMode ? "bg-zinc-800 text-zinc-400" : "bg-gray-100 text-gray-500"}`}>Cancelar</button>
                                <button onClick={() => { handleDelete(d.idd); close(); }} className="flex-1 py-3 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg shadow-rose-500/20 active:scale-95">Eliminar</button>
                              </div>
                            </div>
                          )}
                        </PopoverAnimado>
...
export default DescargoAdministracionPapelera;
