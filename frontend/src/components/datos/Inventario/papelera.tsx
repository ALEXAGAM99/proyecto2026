...
import { Trash2, RotateCcw, Package, AlertCircle, Calendar, Search, X, DollarSign } from "lucide-react";
import { MostrarPapeleraMedicamentos, RestaurarMedicamento, EliminarDefinitivoMedicamento } from "../../conexion/Inventario/Papelera";
...
interface InventarioPapeleraProps { theme: Theme; }

export function InventarioPapelera({ theme }: InventarioPapeleraProps) {
  const { showToast } = useToast();
  const [deleted, setDeleted] = useState<Medicamento[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const isDarkMode = theme === "dark";

  const loadData = async () => {
    try {
      const data = await MostrarPapeleraMedicamentos();
  ...
  const handleRestore = async (id: number) => {
    try { await RestaurarMedicamento(id); showToast("Medicamento restaurado al inventario.", "success"); loadData(); window.dispatchEvent(new Event("update-borrados-count")); }
    catch { showToast("Error al restaurar.", "warning"); }
  };
  const handleDelete = async (id: number) => {
    try { await EliminarDefinitivoMedicamento(id); showToast("Eliminado permanentemente.", "success"); loadData(); window.dispatchEvent(new Event("update-borrados-count")); }
    catch { showToast("Error al eliminar.", "warning"); }
  };

  const filtered = deleted.filter(m =>
    `${m.nomm} ${m.codm} ${m.frecuso}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="flex flex-col h-full p-2 pt-0 overflow-hidden">
      <div className={`flex flex-col md:flex-row justify-between sticky top-0 z-19 items-start md:items-center p-4 gap-4 mb-3 shrink-0 ${isDarkMode ? "bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800" : "bg-white/90 backdrop-blur-md border-b border-gray-100"}`}>
        <div className="flex items-center gap-3 text-red-500">
          <div className="p-3 bg-red-500/10 rounded-2xl"><Package size={26} /></div>
          <div>
            <h1 className="text-2xl font-black tracking-tight">Papelera de Inventario</h1>
            <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${isDarkMode ? "text-zinc-500" : "text-gray-400"}`}>
              {deleted.length} Fármaco{deleted.length !== 1 ? "s" : ""} eliminado{deleted.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        <div className="flex-1 md:flex-none md:w-80 relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={18} />
          <input type="text" placeholder="Buscar en papelera..." className={`${inputBase} pl-12 pr-10`}
            value={searchTerm} onChange={e => setSearchTerm(e.target.value)} list="papelera-inv-list" />
          <datalist id="papelera-inv-list">
            {deleted.slice(-5).reverse().map(m => <option key={m.codm} value={m.nomm} />)}
          </datalist>
          <AnimatePresence>{ (
            <motion.button
            ...
            <tbody>
              <AnimatePresence mode="popLayout">
                {filtered.length > 0 ? filtered.map((m, i) => (
                  <motion.tr key={m.codm} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: i * 0.03 }}
                    className={`border-b last:border-0 ${isDarkMode ? "border-zinc-900 hover:bg-zinc-900/40" : "border-gray-50 hover:bg-gray-50/50"}`}>
                    <td className="px-6 py-5">
                      <span className="text-[11px] font-mono font-bold bg-red-500/10 text-red-500 px-2 py-1 rounded-lg">#{m.codm}</span>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-red-500/10 flex items-center justify-center text-red-400"><Package size={18} /></div>
                        <div>
                          <p className="font-black text-sm tracking-tight">{m.nomm}</p>
                          <p className={`text-[10px] font-bold uppercase opacity-50`}>{m.frecuso || "Sin frecuencia"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className={`text-xs font-black px-2 py-1 rounded-lg ${m.numex <= 5 ? "bg-rose-500/10 text-rose-500" : m.numex <= 15 ? "bg-amber-500/10 text-amber-500" : "bg-zinc-500/10 text-zinc-400"}`}>{m.numex}</span>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className="flex items-center justify-center gap-0.5 text-xs font-black text-emerald-500"><DollarSign size={12} />{Number(m.precio).toFixed(2)}</span>
                    </td>
                    <td className="px-6 py-5">
                      <p className={`text-xs font-bold italic ${isDarkMode ? "text-zinc-500" : "text-gray-400"}`}>{m.UsuarioElimino || "Sistema"}</p>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex flex-col items-center gap-1 text-[10px] font-bold text-zinc-500">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex justify-center gap-2">
                        <PopoverAnimado id={`restore-inv-${m.codm}`} titulo="Restaurar al Inventario" direccion="bl" pWidth={385} pHeight={245} theme={theme} triggerWidth={42} triggerHeight={42}
                          ...
                          {({ close }: any) => (
                            <div className="flex flex-col h-full">
                              <p className={`text-sm mb-6 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}>¿Restaurar <strong>{m.nomm}</strong> al inventario activo?</p>
                              <div className="flex gap-3 mt-auto">
                                <button onClick={() => close()} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest ${isDarkMode ? "bg-zinc-800 text-zinc-400" : "bg-gray-100 text-gray-500"}`}>Cancelar</button>
                                <button onClick={() => { handleRestore(m.codm); close(); }} className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg shadow-emerald-500/20 active:scale-95">Restaurar</button>
                              </div>
                            </div>
                          )}
                        </PopoverAnimado>
                        <PopoverAnimado id={`delete-inv-${m.codm}`} titulo="Eliminar Permanentemente" direccion="bl" pWidth={480} pHeight={245} theme={theme} triggerWidth={42} triggerHeight={42}
                          ...
                          {({ close }: any) => (
                            <div className="flex flex-col h-full">
                              <p className={`text-sm mb-6 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}>¿Eliminar <strong>{m.nomm}</strong> permanentemente? Esta acción no se puede deshacer.</p>
                              <div className="flex gap-3 mt-auto">
                                <button onClick={() => close()} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest ${isDarkMode ? "bg-zinc-800 text-zinc-400" : "bg-gray-100 text-gray-500"}`}>Cancelar</button>
                                <button onClick={() => { handleDelete(m.codm); close(); }} className="flex-1 py-3 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg shadow-rose-500/20 active:scale-95">Eliminar</button>
                              </div>
                            </div>
                          )}
                        </PopoverAnimado>
...
export default InventarioPapelera;
