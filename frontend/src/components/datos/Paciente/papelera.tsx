...
import {
  ...
  RestaurarPaciente,
  EliminarDefinitivoPaciente,
} from "../../conexion/Paciente/Papelera";
import { type Paciente } from "../../conexion/Paciente/Mostrar";
import type { Theme } from "../../sidebars/Sidebar";
import PopoverAnimado from "../../common/PopoverAnimado";
import { useToast } from "../../common/ToastContext";

interface PacientesPapeleraProps {
  theme: Theme;
}

export function PacientesPapelera({ theme }: PacientesPapeleraProps) {
  const { showToast } = useToast();
  const [deleted, setDeleted] = useState<Paciente[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const isDarkMode = theme === "dark";

  const loadData = async () => {
    try {
      const data = await MostrarPapeleraPacientes();
  ...
  const handleRestore = async (ci: number) => {
    try {
      await RestaurarPaciente(String(ci));
      showToast("Paciente restaurado al directorio.", "success");
      loadData();
      window.dispatchEvent(new Event("update-borrados-count"));
    } catch {
      showToast("Error al restaurar.", "warning");
    }
  };
  const handleDelete = async (ci: number) => {
    try {
      await EliminarDefinitivoPaciente(String(ci));
  ...
  return (
    <div className="flex flex-col h-full p-2 pt-0 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 text-red-500">
          ...
          <div>
            ...
            >
              {deleted.length} Registro{deleted.length !== 1 ? "s" : ""}{" "}
              eliminado{deleted.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        <div className="flex-1 md:flex-none md:w-80 relative">
          ...
          <input
            ...
            list="papelera-pac-list"
          />
          <datalist id="papelera-pac-list">
            {deleted
              .slice(-5)
              .reverse()
              .map((p) => (
      ...
      <div
        className={`border rounded-[2.5rem] flex-1 min-h-0 overflow-hidden ${isDarkMode ? "bg-zinc-950 border-zinc-900" : "bg-white border-gray-100 shadow-sm"}`}
      >
        <div className="h-full overflow-y-auto custom-scrollbar">
          <table className="w-full text-left border-collapse">
            <thead
              className={`sticky top-0 z-19 ${isDarkMode ? "bg-zinc-900" : "bg-gray-50"}`}
            >
                    ...
                    >
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm ${isDarkMode ? "bg-zinc-800 text-zinc-500" : "bg-gray-100 text-gray-400"}`}
                          >
                            {p.nom.charAt(0)}
                            {p.ap.charAt(0)}
                          </div>
                      ...
                      <td className="">
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded-lg ${isDarkMode ? "bg-zinc-900 text-zinc-400" : "bg-gray-50 text-gray-500"}`}
                        >
                          {p.edad} años
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-col items-center gap-1 text-[10px] font-bold text-zinc-500">
                          <div className="flex items-center gap-1.5">
                            ...
                            {p.FechaEliminacion
                              ? new Date(p.FechaEliminacion).toLocaleDateString(
                                  "es-ES",
                                  {
                                    day: "2-digit",
                                    month: "2-digit",
                          ...
                          <span className="opacity-50">
                            {p.FechaEliminacion
                              ? new Date(p.FechaEliminacion).toLocaleTimeString(
                                  "es-ES",
                                  {
                                    hour: "2-digit",
                                    minute: "2-digit",
                            ...
                            {({ close }: any) => (
                              <div className="flex flex-col h-full">
                                ...
                                <div className="flex gap-3 mt-auto">
                                  <button
                                    onClick={() => close()}
                                    className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest ${isDarkMode ? "bg-zinc-800 text-zinc-400" : "bg-gray-100 text-gray-500"}`}
                                  >
                                    Cancelar
                                  </button>
                                  <button
                                    ...
                                    className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95"
                                  >
                                    Restaurar
                                  </button>
                                </div>
                            ...
                            {({ close }: any) => (
                              <div className="flex flex-col h-full">
                                ...
                                >
                                  ...
                                  de forma permanente? Esta acción no se puede
                                  deshacer.
                                </p>
                                <div className="flex gap-3 mt-auto">
                                  <button
                                  ...
                                  <button
                                    ...
                                    className="flex-1 py-3 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-500/20 active:scale-95"
                                  >
                                    Eliminar
                                  </button>
                                </div>
...
export default PacientesPapelera;
